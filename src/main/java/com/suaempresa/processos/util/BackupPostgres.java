...
public class BackupPostgres {
    ...
    public static void realizarBackupMensal(String host, String port, String database, String user, String password, String backupDir) {
        ...
        processBuilder.redirectErrorStream(true); // Junta stdout e stderr

        System.out.println("Comando executado: pg_dump -h " + host + " -p " + port + " -U " + user + " -F p -b -v -f \"" + caminhoArquivo + "\" " + database);
        ...
        try {
            ...
            // Exibir a saída do processo (stdout + stderr)
            new Thread(() -> {
